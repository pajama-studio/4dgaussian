//! cargo run --example native_smoke -- [model.ply[.gz]] [cameras.json] [output-dir]
//! Native GPU smoke evidence, not a throughput benchmark or a held-out metric.
...
async fn run() -> Result<(), Box<dyn Error>> {
    ...
    let mut splats = StgPass::new(&device, &ply, format, None)?;
    ...
    for (index, time) in [0.1_f32, 0.5, 0.9].into_iter().enumerate() {
        ...
        let changed = previous.as_ref().map(|p| {
            p.chunks_exact(4)
        ...
        if changed == Some(0) {
            return Err("Dynamic fixture did not change across timestamps".into());
        }
        let filename = output.join(format!("frame-{index}.png"));
        ...
        reports.push(serde_json::json!({
            "normalizedTime": time, "source": stats.source_count, "visible": stats.visible_count,
            "indexUploadBytes": stats.index_upload_bytes, "prepareMs": prepare_ms,
            "coloredPixels": colored, "changedPixelsFromPrevious": changed, "image": filename,
        }));
    }
