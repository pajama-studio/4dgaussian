...
impl GaussianRenderer {
    ...
    pub fn render(
        ...
        let inspection_frame = inspection::InspectionFrame {
            ...
            view_proj,
            viewport,
            eye: view.inverse().transform_point3(Vec3::ZERO),
        };
    ...
    pub fn splat_record(&self, id: u32) -> Vec<f32> {
        self.inspection_records
            .get(id as usize)
            .map_or_else(Vec::new, |r| r.to_vec())
    }

    #[wasm_bindgen(js_name = inspectSplat)]
    pub fn inspect_splat(&self, id: u32) -> Vec<f32> {
        match (
            self.inspection_records.get(id as usize),
            self.inspection_frame.as_ref(),
        ) {
            (Some(row), Some(frame)) => {
                inspection::project(row, frame).map_or_else(Vec::new, |s| s.pack(frame))
            }
            _ => vec![],
        }
    }
...
fn parse_stg_ply(bytes: &[u8]) -> Result<ParsedPly, String> {
    ...
    if count == 0 || count > MAX_SPLATS {
        return Err(format!(
            "Expected 1..={MAX_SPLATS} STG-Lite splats, found {count}"
        ));
    }
    let payload_end = count
    ...
    for (index, record) in payload.chunks_exact(BYTES_PER_SPLAT).enumerate() {
        let value = |index: usize| {
            let offset = index * 4;
            f32::from_le_bytes(record[offset..offset + 4].try_into().unwrap())
        };
        if (0..FLOATS_PER_SPLAT).any(|field| !value(field).is_finite()) {
            return Err(format!(
                "STG-Lite record {index} contains a non-finite value"
            ));
        }
        if [4, 21, 22, 23]
            .iter()
            .any(|&field| !value(field).exp().is_finite())
        {
            return Err(format!(
                "STG-Lite record {index} has an overflowing log scale"
            ));
        }
...
mod tests {
    ...
    #[test]
    fn rejects_non_finite_attributes_and_oversized_counts_before_allocation() {
        let mut bytes = fixture_ply();
        let offset = parse_stg_ply(&bytes).unwrap().payload_offset;
        bytes[offset..offset + 4].copy_from_slice(&f32::NAN.to_le_bytes());
        assert!(matches!(parse_stg_ply(&bytes), Err(message) if message.contains("non-finite")));
        let bytes = fixture_ply();
        let header = String::from_utf8_lossy(&bytes[..offset])
            .replace("element vertex 1", "element vertex 18446744073709551615");
        assert!(parse_stg_ply(header.as_bytes()).is_err());
    }
}
